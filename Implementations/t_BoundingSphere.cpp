...
BOOST_AUTO_TEST_CASE(BoundingSphereConstructors)
{
    ...
    BOOST_CHECK_EQUAL(sphere1.GetCenter().GetX(), 0.0f);
    BOOST_CHECK_EQUAL(sphere1.GetCenter().GetY(), 0.0f);
    BOOST_CHECK_EQUAL(sphere1.GetCenter().GetZ(), 0.0f);
    BOOST_CHECK_EQUAL(sphere1.GetRadius(), 0.0f);

    TC::BoundingSphere sphere2 { TM::Vector3(2.0f, 5.0f, 1.0f), 25.0f };

    BOOST_CHECK_EQUAL(sphere2.GetCenter().GetX(), 2.0f);
    BOOST_CHECK_EQUAL(sphere2.GetCenter().GetY(), 5.0f);
    BOOST_CHECK_EQUAL(sphere2.GetCenter().GetZ(), 1.0f);
    BOOST_CHECK_EQUAL(sphere2.GetRadius(), 25.0f);

    //Combination test will be more difficult
    //TC::BoundingSphere sphere3 { TM::Vector3() }
}

BOOST_AUTO_TEST_CASE(BoundingSphereAccessors)
{
    TC::BoundingSphere sphere { TM::Vector3(10.0f, 10.0f, 0.0f), 5.0f };

    BOOST_CHECK_EQUAL(sphere.GetCenter().GetX(), 10.0f);
    BOOST_CHECK_EQUAL(sphere.GetCenter().GetY(), 10.0f);
    BOOST_CHECK_EQUAL(sphere.GetCenter().GetZ(), 0.0f);
    BOOST_CHECK_EQUAL(sphere.GetRadius(), 5.0f);

    sphere.SetCenter(TM::Vector3(35.0f, 1.0f, 2.0f));
    sphere.SetRadius(2.5f);

    BOOST_CHECK_EQUAL(sphere.GetCenter().GetX(), 35.0f);
    BOOST_CHECK_EQUAL(sphere.GetCenter().GetY(), 1.0f);
    BOOST_CHECK_EQUAL(sphere.GetCenter().GetZ(), 2.0f);
    BOOST_CHECK_EQUAL(sphere.GetRadius(), 2.5f);

    sphere.SetCenter(26.0f, 15.0f, 23.0f);

    BOOST_CHECK_EQUAL(sphere.GetCenter().GetX(), 26.0f);
    BOOST_CHECK_EQUAL(sphere.GetCenter().GetY(), 15.0f);
    BOOST_CHECK_EQUAL(sphere.GetCenter().GetZ(), 23.0f);
}

BOOST_AUTO_TEST_CASE(BoundingShereIntersectBoundingSphere)
{
    TC::BoundingSphere sphere1 { TM::Vector3(0.0f, 0.0f, 0.0f), 4.0f };
    TC::BoundingSphere sphere2 { TM::Vector3(2.0f, 2.0f, 2.0f), 4.0f };
...
BOOST_AUTO_TEST_CASE(BoudningSphereGetGrowth)
{
    ...
    TC::BoundingSphere sphere1 { TM::Vector3(0.0f, 0.0f, 0.0f), 8.0f };
    TC::BoundingSphere sphere2 { TM::Vector3(0.0f, 0.0f, 0.0f), 10.0f };

    //Check if the growth from 2 would be bigger than 1 is now. 
    BOOST_CHECK_GT(sphere1.GetGrowth(sphere2), sphere1.GetRadius());

    TC::BoundingSphere sphere3 { TM::Vector3(10.0f, 10.0f, 0.0f), 10.0f };
    ...
    TC::BoundingSphere sphere4 { TM::Vector3(5.0f, 5.0f, 0.0f), 2.0f };
